...
export default function SettingsScreen() {
  ...
  return <><AppScreen><SubpageHeader title="Language" subtitle="App language preferences" onBack={back} /><View style={styles.sections}><SurfaceCard><ProfileMenuItem icon="check-circle" label="English" supportingText="Current app language" trailingText={selectedLanguage === 'en' ? 'Selected' : undefined} onPress={() => setLanguage('en')} /><ProfileMenuItem icon="clock-outline" label="Malayalam" supportingText="Localization is planned for a future release" trailingText="Coming later" /></SurfaceCard>{demoConfig.resetEnabled ? <ProfileSection title="Developer Demo"><View style={styles.demo}><AppText variant="bodySmall" color={colors.neutral.textSecondary}>Restore local Session progress, updates, attendance, assessments, training plans, and preferences without signing out.</AppText><AppButton testID="reset-demo-data" label="Reset demo data" variant="secondary" onPress={() => setResetVisible(true)} accessibilityLabel="Reset frontend demo data" /></View></ProfileSection> : null}</View></AppScreen><AppConfirmationDialog visible={resetVisible} icon="restore-alert" title="Reset demo data?" description="Session progress, update read state, attendance, assessments, training plans, and preferences will return to their defaults. You will remain signed in." cancelLabel="Keep Demo Data" confirmLabel="Reset Demo Data" destructive loading={resetting} onCancel={() => setResetVisible(false)} onConfirm={() => { void resetDemo(); }} /></>;
}
