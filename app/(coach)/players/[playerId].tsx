...
export default function CoachPlayerDetailScreen() {
  ...
  const openHandoff = (action: CoachPlayerAction) => navigateOnce(() => router.push({ pathname: '/(coach)/players/[playerId]/action/[action]', params: { playerId: player.id, action } }));
  const attendanceSessionId = getDefaultAttendanceSessionForSquad(player.squadId)?.id ?? 'training-1';
  const quickAction = (action: CoachPlayerAction | 'attendance') => { if (action === 'attendance') navigateOnce(() => router.push({ pathname: '/(coach)/(tabs)/attendance', params: { sessionId: attendanceSessionId, playerId: player.id } })); else if (action === 'feedback') navigateOnce(() => router.push({ pathname: '/(coach)/feedback', params: { playerId: player.id } })); else if (action === 'progress' && assessment) navigateOnce(() => router.push({ pathname: '/(coach)/feedback/[assessmentId]', params: { assessmentId: assessment.id } })); else openHandoff(action); };
  const openLesson = (lesson: LearningLesson) => navigateOnce(() => router.push({ pathname: '/(coach)/sessions/[sessionId]', params: { sessionId: lesson.id, playerId: player.id } }));
  return <AppScreen withTabBarClearance={false}><SubpageHeader title="Player Detail" subtitle={player.category} onBack={back} /><View style={styles.sections}><FadeInView translate={false}><CoachPlayerIdentityCard player={player} /></FadeInView><ProfileSection title="Quick Actions"><PlayerQuickActions onAction={quickAction} /></ProfileSection><ProfileSection title="Current Snapshot"><PlayerSnapshotCard player={player} assessment={assessment} onAttendance={() => navigateOnce(() => router.push({ pathname: '/(coach)/(tabs)/attendance', params: { sessionId: attendanceSessionId, playerId: player.id } }))} onProgress={() => quickAction('progress')} /></ProfileSection><ProfileSection title="Current Goal"><PlayerGoalCard player={player} /></ProfileSection><ProfileSection title="Latest Feedback">{feedback.length ? <PlayerFeedbackCard feedback={feedback} coachName={data.coach.name} onHistory={() => navigateOnce(() => router.push({ pathname: '/(coach)/feedback/history', params: { playerId: player.id } }))} /> : <ContentState type="empty" title="No recent feedback" message="Coach feedback will appear here after review." />}</ProfileSection><ProfileSection title="Current Assigned Session">{lessons.length ? <AssignedLessonsCard lessons={lessons} onOpen={openLesson} /> : <ContentState type="empty" title="No assigned Session" message="Assign a Session while adding feedback or preparing a Session plan." />}</ProfileSection><View><AppButton label={showMore ? 'Hide More Information' : 'More Information'} variant="secondary" onPress={() => setShowMore((current) => !current)} accessibilityLabel={showMore ? 'Hide academy and player information' : 'Show more academy and player information'} />{showMore ? <View style={styles.more}><CoachPlayerAcademyDetails player={player} squad={squad} coachName={data.coach.name} ground={data.todaySession.ground} /></View> : null}</View></View></AppScreen>;
}
