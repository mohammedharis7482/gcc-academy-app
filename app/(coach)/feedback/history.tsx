...
export default function CoachFeedbackHistoryScreen() {
  ...
  return <SafeAreaView style={styles.safe} edges={['top']}><FlatList data={filtered} keyExtractor={(item) => item.id} renderItem={({ item }) => <AssessmentHistoryRow assessment={item} onPress={() => navigateOnce(() => router.push({ pathname: '/(coach)/feedback/[assessmentId]', params: { assessmentId: item.id } }))} />} ItemSeparatorComponent={Separator} ListHeaderComponent={header} ListEmptyComponent={<ContentState type="empty" title="No feedback found" message="Published feedback matching this filter will appear here." />} contentContainerStyle={[styles.content, { paddingBottom: insets.bottom + layout.tabClearance }]} showsVerticalScrollIndicator={false} initialNumToRender={12} /></SafeAreaView>;
}
