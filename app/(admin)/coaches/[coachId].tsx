...
export default function AdminCoachDetailScreen() {
  ...
  const salary = admin.getCoachSalaries(adminDemoConfig.billing.currentPeriod).find((item) => item.coachId === coach.id);
  ...
  return <AppScreen withTabBarClearance={false}>
    ...
    <View style={styles.sections}>
      ...
      <ProfileSection title="Assignment"><SurfaceCard><InfoRow icon="briefcase-outline" label="Engagement" value={coach.engagement} /><InfoRow icon="calendar-account-outline" label="Availability" value={coach.availability === 'available' ? 'Available for sessions' : 'On leave'} /><InfoRow icon="account-multiple-outline" label="Players managed" value={`${managedPlayers} players`} /><InfoRow icon="clipboard-text-clock-outline" label="Sessions this month" value={String(coach.sessionsThisMonth)} /><InfoRow icon="cash-multiple" label="Monthly salary" value={formatCurrency(coach.monthlySalary)} /><InfoRow icon={salary?.status === 'paid' ? 'cash-check' : 'cash-clock'} label={`${adminDemoConfig.billing.currentPeriod} salary`} value={salary?.status === 'paid' ? `Paid ${salary.paidOn ?? ''}` : 'Pending'} /><InfoRow icon="certificate-outline" label="Certification" value={coach.certification} /><InfoRow icon="calendar-start" label="Joined" value={coach.joinedOn} /></SurfaceCard></ProfileSection>
