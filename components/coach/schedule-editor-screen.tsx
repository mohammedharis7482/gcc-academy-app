...
export function ScheduleEditorScreen({ scheduleId }: { readonly scheduleId?: string }) {
  ...
  const publish = async () => {
    ...
    publishingRef.current = true; setError(undefined); const categoryName = category === 'U13' ? demoConfig.player.category : `${category} ${category === 'U10' ? 'Foundation' : 'Performance'} Squad`;
    try {
