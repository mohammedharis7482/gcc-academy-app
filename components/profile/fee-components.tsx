...
export function CurrentFeeCard({ fee }: { fee: FeeRecord | null }) {
  if (!fee) return <SurfaceCard compactRows={false} accessibilityLabel="No pending academy fee"><View style={styles.empty}><View style={styles.success}><MaterialCommunityIcons name="check" size={25} color={colors.status.success} /></View><View style={styles.grow}><AppText variant="heading" weight="extraBold">No pending fee</AppText><AppText variant="bodySmall" color={colors.neutral.textSecondary}>Academy fee records are currently up to date.</AppText></View></View></SurfaceCard>;
  return <View style={styles.current} accessibilityLabel={`${fee.period}, ${formatCurrency(fee.amount)}, ${fee.status}, due ${fee.dueDate}`}><View style={styles.currentTop}><View><AppText variant="caption" weight="bold" color={colors.neutral.textSecondary}>CURRENT FEE</AppText><AppText variant="heading" weight="extraBold">{fee.period}</AppText></View><StatusBadge label={fee.status} tone={fee.status === 'overdue' ? 'error' : 'warning'} /></View><AppText variant="display" weight="extraBold" color={colors.brand.navy}>{formatCurrency(fee.amount)}</AppText><View style={styles.due}><MaterialCommunityIcons name="calendar-alert" size={18} color={colors.status.warning} /><AppText variant="bodySmall" weight="semibold">Due {fee.dueDate}</AppText></View></View>;
}
